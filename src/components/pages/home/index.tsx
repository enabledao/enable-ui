    ...
    componentDidMount = async () => {
        ...
        const contributors = (await Promise.all(
            Array.from(
                ...
                const contributions = fundEvents
                    .filter(event => event.returnValues.sender === address)
                    .sort((a, b) => (a.blockNumber > b.blockNumber ? 1 : 0))
                const lastContribution = (await getBlock(
                    contributions[contributions.length - 1].blockHash
                )).timestamp
